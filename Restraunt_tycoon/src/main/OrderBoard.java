...
public class OrderBoard {
     ...
     * @param gp The game panel instance to be used for displaying orders and interacting with the game.
     */
    ...
    /**
     * Loads the order board with the current customers waiting at a specific stall. It clears any existing orders and populates
     *  the board with the orders of customers waiting for the specified stall type.
    ...
    /**
     * Fulfills the first available item from the waiting customers. It iterates through the customers and their orders, checking if the player has the required item in their inventory.
     * If an item is fulfilled, it updates the customer's order and gives the player money for selling the item. If a customer's order is completely fulfilled, they are removed from the 
     * waiting list.
     */
    ...
    /**
     * Helper method to convert an item name from the order into the corresponding index in the player's inventory. 
     * It uses a switch statement to map item names to their respective inventory indices.
     * @param name The name of the item to convert
     * @return The index of the item in the player's inventory, or -1 if not found
     */
    ...
    /**
     * Draws the order board on the screen if it is visible. It renders a semi-transparent background with a border, 
     * and displays the orders of each waiting customer in separate columns.
